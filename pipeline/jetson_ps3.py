...
# Option 1: HDMI Capture (HD60 S+ - RECOMMENDED FOR S3 
USE_HD60S_CAPTURE = True      # Set to True when using HD60 S+ 
HD60S_DEVICE_ID = 1          # This will be auto-detected from your working test
HD60S_RESOLUTION = (1920, 1080)  # PS3 outputs 1080p
HD60S_FPS = 30               # Standard PS3 output
...
WINDOW_NAME = "PS3Switch Real-Time Translation - Jetson Orin"

# BALANCED SETTINGS FOR RELIABLE TRANSLATION
TARGET_FPS = 25              # Realistic target for PS3
...
BASE_DIR = "/workspace/pipeline/results/jetson_ps3"
OUTPUT_VIDEO = os.path.join(BASE_DIR, "ps3_translation.mp4")
FPS_GRAPH_PNG = os.path.join(BASE_DIR, "fps_ps3.png")
...
def setup_hd60s_capture():
    """Setup HD60 S+ capture for PS3 on Jetson"""
    ...
    print(f"Configuring HD60 S+ (Device {working_device_id}) for PS3...")
    ...
    print(f"HD60 S+ configured for PS3:")
...
# Initialize models
print("JETSON PS3 INITIALIZATION")
print("="*50)
...
def ps3_hud(frame, fps_disp, overlay_count, queue_depths, stats):
    """PS3 HUD - clean interface"""
    # Performance metrics
    ...
    cv2.putText(frame, f"PS3", (10,110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
...
def run():
    ...
    print("Processing PS3 frames... (Press 'q' to quit)")
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
            ...
            frame = ps3_hud(frame, fps_disp, overlay_count, queue_depths, stats)
        ...
        if fps_list:
            ...
            print(f"\nPS3 Translation Performance Summary:")
