...
def fixed_hud(frame, fps_disp, overlay_count, queue_depths, stats):
    """Fixed HUD with debug info - REMOVED pipeline text and GPU display from screen"""
    # Performance metrics
...
def run():
    ...
    print("Processing video with fixed translation... (Press 'q' to quit)")
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
            ...
            # Progress update - GPU info still printed to terminal
        ...
        if fps_list:
            ...
            print(f"\nFixed Translation Performance Summary:")
