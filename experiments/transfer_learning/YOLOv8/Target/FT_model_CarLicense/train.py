...
def main():
    ...
    source_model_path = r"F:\FAU\Thesis\HDMIBabelfishV2\experiments\transfer_learning\YOLOv8\Source\Scratch_model\runs\detect\train\weights\best.pt"
    model = YOLO(source_model_path)
    # Uncomment below if you want to train from scratch (using the yaml configuration) instead
    # model = YOLO('yolov8s.yaml')
    
    print("Starting training on target dataset using source model...")
    model.train(data=dataset_yaml_path, epochs=10, imgsz=640, batch=16)
    

    target_dir = r"F:\FAU\Thesis\HDMIBabelfishV2\experiments\transfer_learning\YOLOv8\Target\FT_model_CarLicense"
