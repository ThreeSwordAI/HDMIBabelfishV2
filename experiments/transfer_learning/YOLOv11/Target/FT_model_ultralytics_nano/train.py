...
def main():
    ...
    # Load a pretrained YOLOv8 nano model (fine-tuning).
    model = YOLO('yolo11n.pt')
    # Uncomment the following line to train from scratch:
    # model = YOLO('yolov11n.yaml')
    
    print("Starting training on new dataset...")
    model.train(data=dataset_yaml_path, epochs=30, imgsz=416, batch=16) 
    #As the model is small. epochs are set to 30. and also image size is less beacuse of fast inferernce target.
    
  
    target_dir = r"F:\FAU\Thesis\HDMIBabelfishV2\experiments\transfer_learning\YOLOv11\Target\FT_model_ultralytics_nano"
