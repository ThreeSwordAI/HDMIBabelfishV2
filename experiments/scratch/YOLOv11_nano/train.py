...
def save_prediction_images(model, test_dir, prediction_dir, count=15):
    ...
    model.predict(source=sampled_images, imgsz=416, conf=0.25, save=True, project=prediction_dir, name="predictions", exist_ok=True)
...
def main():
    ...
    model.train(data=dataset_yaml_path, epochs=50, imgsz=416, batch=16)
    

    target_dir = r"F:\FAU\Thesis\HDMIBabelfishV2\experiments\scratch\YOLOv11_nano"
