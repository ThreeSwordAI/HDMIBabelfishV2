...
# -------- CONFIG --------
VIDEO_PATH = "../data/test_video/JapanRPG_TestSequence.mov"  # Path from pc folder
...
WINDOW_NAME = "RealTime Translate - CUDA - Marian"
TARGET_FPS = 30  # Higher FPS for CUDA
STABILITY_MS = 300  # Slightly reduced for better responsiveness
...
def run():
    print("\n" + "="*50)
    print("🚀 STARTING CUDA VIDEO PROCESSING")
    print("="*50)
    
    # Use local variable for video path
    ...
    if not video_path.exists():
        ...
        found_video = False
        for alt_video in alternative_videos[:-1]:  # Skip camera for now
        ...
        if not found_video:
            print("⚠ No video file found. You can:")
            print("  1. Place a video file in ../data/test_video/")
            print("  2. Use camera input (set VIDEO_PATH = 0)")
            print("  3. Provide the correct video path")
            return
    ...
    if not cap.isOpened():
        ...
        if not cap.isOpened():
            print("❌ Could not open camera either")
            return
        else:
    ...
    while True:
        ...
        cv2.putText(frame, f"Model: Marian", (10,110),
